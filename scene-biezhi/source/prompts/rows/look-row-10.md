Create one horizontal look-direction strip for Codex pet `scene-biezhi`, atlas row 10.
...
Place one centered pose in each invisible equal-width slot on flat pure cyan #00FFFF. Change only the natural parts needed to express gaze: eyes, eyelids, head, face, neck, upper body, appendages, and constrained prop follow-through. Keep identity, silhouette, materials, palette, markings, and props consistent.
